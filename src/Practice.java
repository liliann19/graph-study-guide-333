...
public class Practice {
  ...
  private static int oddVertices(Vertex<Integer> starting, Set<Vertex<Integer>> visited) {
    ...
    for (Vertex<Integer> neighbor : starting.neighbors) {
      countOdd += oddVertices(neighbor, visited);
    }
    return countOdd;
  }
  ...
  public static List<Integer> sortedReachable(Vertex<Integer> starting) {
    List<Integer> result = sortedReachable(starting, new HashSet<>());
  ...
  private static boolean positivePathExists(Map<Integer, Set<Integer>> graph, int starting, int ending, Set<Integer> visited) {
    if (!graph.containsKey(starting) || starting <= 0 || visited.contains(starting)) return false;
  ...
  public static List<int[]> nextMoves(char[][] board, int[] current, int[][] directions) {
    ...
    for (int[] direction : directions) {
      ...
      if (newRow >= 0 && newRow < board.length && newCol >= 0 && newCol < board[0].length && board[newRow][newCol] != 'X') {
